...
import { $ as $$PCountryDropdown, a as $$PLocaleDropdown } from '../../../chunks/PLocaleDropdown_avF037v5.mjs';
    ...
    new Set(
      allProjects.flatMap((project) => {
        ...
        if (Array.isArray(pCountry)) {
          return pCountry.filter((country) => country && String(country).trim() !== "");
        } else if (pCountry && String(pCountry).trim() !== "") {
          return [pCountry];
        }
        return [];
      })
    )
  ).map((country) => String(country).toLowerCase().replace(/\s+/g, "-"));
  const paths = countries.map((country) => {
    const countryProjects = allProjects.filter((project) => {
      const pCountry = project.data.properties.pCountry;
...
const $$ = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "MainLayout", $$MainLayout, { "title": `Projects in ${readableCountry} - Page ${page.currentPage} of ${page.lastPage}` }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<section class="bg-white dark:bg-gray-900"> <div class="py-8 px-4 mx-auto max-w-screen-xl sm:py-16 lg:px-6"> <div class="max-w-screen-md mb-8 lg:mb-16"> <h1 class="mb-4 text-4xl tracking-tight font-extrabold text-gray-900 dark:text-white">
Projects in ${readableCountry} </h1> <p class="text-gray-500 sm:text-xl dark:text-gray-400">
...
}, "/workspace/src/pages/projects/[country]/[...page].astro", void 0);

const $$file = "/workspace/src/pages/projects/[country]/[...page].astro";
const $$url = "/projects/[country]/[...page]";
